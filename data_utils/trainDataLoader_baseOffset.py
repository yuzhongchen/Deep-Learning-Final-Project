...
class trainDataLoader(Dataset):
    ...
    def _get_item(self, index):
        ...
        gt_xyzs=np.zeros((self.maxclip,self.max_it,8,3)) 
            ...
            for it in range(self.max_it):
                for dirc in range(8):
                    ...
                    if gt_xyz[idx,2]>centers[idx,it,dirc,2]:
                        z=1
                    gt_xyzs[idx,it,dirc,:]= gt_xyz[idx,:]-centers[idx,it,dirc,:]
                    if it==0:
                        x=floor((dirc)/4)
                        z=(dirc)%2
                    ...
                    z=z*2-1
                    if(it<self.max_it-1):
                        centers[idx,it+1,dirc,:]=np.asarray([centers[idx,it,dirc,0]+x*0.3*0.7**(it),centers[idx,it,dirc,1]+y*0.3*0.7**(it),centers[idx,it,dirc,2]+z*0.3*0.7**(it)])
