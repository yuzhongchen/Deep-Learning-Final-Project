...
class testDataLoader(Dataset):
    def __init__(self, root,num):
        ...
        self.gtpath=os.path.join(root,'annotest')
        self.num=num
                    ...
                    if len(line)==9:
                        for ran in range(2):
                            if ran==0:
                                ...
                                self.maxclip_grab=max(self.maxclip_grab,int(int(line[1])-int(line[0])))
                            else:
                                self.indexlist_release.append([scene,txtnum[:-4],line[1],line[2],line[6:]])
                                self.maxclip_release=max(self.maxclip_release,int(int(line[2])-int(line[1])))
        ...
        self.length=max(len(self.indexlist_grab),len(self.indexlist_release))
        self.maxclip = max(self.maxclip_grab,self.maxclip_release)
    ...
    def _get_item_grab(self, index):        
        finalsource=self.indexlist_grab[index]   
                    ...
                    if results.multi_hand_landmarks is not None:
                        for landmarks in results.multi_hand_landmarks:
                            ...
                            for lm in landmarks.landmark:
                                positions[idx,id] = lm.x
                                positions[idx,id+1] = lm.y
                                positions[idx,id+2] = lm.z
                                id+=3
                else:
        ...
        return gt_xyz,pointcloud,geometry,rangenum,finalsource,positions

    def _get_item_release(self, index):        
        finalsource=self.indexlist_release[index]   
        imupath=os.path.join(self.scenepath,finalsource[0],finalsource[1],'data.txt')
        videopath=os.path.join(self.scenepath,finalsource[0],finalsource[1],'rgb_video.mp4')
